...
const MemoBoardPage = () => {
  ...
  const handleAddItem = () => {
    ...
    const newMemoItem = { id: Date.now(), content: newItem, isChecked: false };
    setMemo({ ...memo, items: [...(memo.items || []), newMemoItem] });
    setNewItem('');
  };
  ...
  return (
    <div className="font-body text-slate-900 corkboard-bg min-h-screen">
      ...
      <main className="pt-28 pb-32 px-4 flex justify-center items-center min-h-[calc(100vh-80px)]">
        {/* Giant Post-it */}
        <div 
        ...
        >
          ...
          <div className="flex-grow space-y-4 mb-8">
            {memo?.items && memo.items.length > 0 ? (
              memo.items.map((item) => (
                <div key={item.id} className="flex items-center group py-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-4 cursor-pointer shadow-sm transition-all ${item.isChecked ? 'bg-emerald-500' : 'border-2 border-emerald-300 hover:border-emerald-500'}`}>
                    {item.isChecked && <span className="material-symbols-outlined text-white text-xl">check</span>}
                  </div>
                  <span className={`text-xl md:text-2xl flex-grow font-medium ${item.isChecked ? 'line-through text-slate-400 decoration-slate-400 decoration-2' : 'text-slate-800'}`}>
                    {item.content}
                  </span>
                </div>
              ))
            ) : (
              <div className="text-center py-20 text-slate-400 italic font-medium text-2xl">
                아직 등록된 할 일이 없어요. 아래에서 추가해보세요!
              </div>
            )}
          </div>
